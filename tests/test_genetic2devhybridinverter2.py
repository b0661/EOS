...
Module under test
-----------------
``akkudoktoreos.devices.genetic2.hybridinverter2``
...
Covers
------
    ContinuousInverterParam — validation
        - Identical set of constraints as HybridInverterParam

    ContinuousInverterParam — derived properties
        - battery_min/max_soc_wh
        - No valid_modes / n_modes (not needed)

    ContinuousInverterDevice — topology and identity
...
from akkudoktoreos.devices.genetic2.hybridinverter2 import (
    ContinuousInverterBatchState,
    ContinuousInverterDevice,
    ContinuousInverterParam,
    InverterType,
)
...
def make_battery_param(
    ...
    return ContinuousInverterParam(
        device_id=device_id,
        port_id="p_ac",
        bus_id="bus_ac",
...
def make_device(
    ...
    device = ContinuousInverterDevice(param, device_index, port_index)
...
# ============================================================
# TestContinuousInverterParamValidation
# ============================================================

class TestContinuousInverterParamValidation:
...
# ============================================================
# TestContinuousInverterParamDerivedProperties
# ============================================================

class TestContinuousInverterParamDerivedProperties:
...
# ============================================================
# TestContinuousInverterDeviceTopology
# ============================================================

class TestContinuousInverterDeviceTopology:
...
class TestSetupRun:
    ...
    def test_stores_step_times(self):
        ...
        dev = ContinuousInverterDevice(make_battery_param(), 0, 0)
        dev.setup_run(ctx)
    ...
    def test_solar_pv_key_none_raises(self):
        ...
        with pytest.raises(ValueError, match="pv_power_w_key"):
            ContinuousInverterDevice(param, 0, 0).setup_run(make_context())

    def test_pv_wrong_shape_raises(self):
        dev = ContinuousInverterDevice(make_solar_param(), 0, 0)
        ctx = FakeContext(make_step_times(4), pv_w=np.full(2, 3000.0))
        with pytest.raises(ValueError):
            dev.setup_run(ctx)

    def test_initial_soc_out_of_bounds_raises(self):
        dev = ContinuousInverterDevice(make_battery_param(min_soc=0.2, max_soc=0.8), 0, 0)
...
class TestGenomeRequirements:
    ...
    def test_before_setup_run_raises(self):
        with pytest.raises(RuntimeError):
            ContinuousInverterDevice(make_battery_param(), 0, 0).genome_requirements()
...
class TestCreateBatchState:
    ...
    def test_before_setup_run_raises(self):
        with pytest.raises(RuntimeError):
            ContinuousInverterDevice(make_battery_param(), 0, 0).create_batch_state(POP, HORIZON)
...
class TestBuildDeviceRequest:
    ...
    def test_idle_steps_zero_min_energy(self):
        device = ContinuousInverterDevice(make_battery_param(), 0, 0)
        device.setup_run(make_context(horizon=HORIZON))
    ...
    def test_charge_steps_positive_min_energy(self):
        device = ContinuousInverterDevice(
