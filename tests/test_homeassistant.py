...
class TestHomeAssistantAddon:
    ...
    @pytest.mark.docker
    def test_validate_addon_with_hassfest(self):
        """Run Home Assistant's hassfest validation using Docker.

        Note: This test is primarily designed for CI environments.
        For local validation, use the GitHub Actions workflow or run basic checks.
        """
        # Skip if in CI environment
        if os.environ.get("CI"):
            pytest.skip(
                "Hassfest validation is best run in GitHub Actions flow in CI."
            )
        ...
        print(f"\nRunning hassfest validation on: {self.root}")

        # Use the hassfest action container directly
        result = subprocess.run(
            [
                "docker", "run", "--rm",
                "-v", f"{self.root}:/github/workspace",
                "ghcr.io/home-assistant/amd64-hassio-supervisor:latest",
                "python3", "-m", "supervisor.addons.validate", "/github/workspace"
            ],
            capture_output=True,
            text=True,
            cwd=str(self.root),
        ...
        # Try alternative if first method fails
        if result.returncode != 0 and "No module named" in result.stderr:
            print("Trying alternative hassfest validation method...")

            # Use the GitHub action container
            result = subprocess.run(
                [
                    "docker", "run", "--rm",
                    "-v", f"{self.root}:/data",
                    "-e", "INPUT_PATH=/data",
                    "ghcr.io/home-assistant/actions/hassfest:latest"
                ],
                capture_output=True,
                text=True,
                cwd=str(self.root),
        ...
        if result.returncode != 0:
            # If both methods failed, it might be an issue with the validation tool itself
            if "No module named" in result.stderr or "unknown" in result.stderr.lower():
                pytest.skip(
                    "Hassfest validation tool not available or incompatible. "
                    "Use GitHub Actions for official validation."
                )

            pytest.fail(
                f"Hassfest validation failed with exit code {result.returncode}\n"
                f"This means the add-on configuration doesn't meet Home Assistant standards."
            )
