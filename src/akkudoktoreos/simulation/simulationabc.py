"""Abstract and base classes for simulation.

Simulation is leaned on the S2 standard. Resource Manager became Simulation.

This module provides data models and enums for energy resource management simulation
...
class Transition(ParametersBaseModel):
    ...
    transition_costs: Optional[float] = Field(
        None,
        description=(
            "Absolute costs for going through this Transition, in the currency defined in SimulationDetails."
        ),
    )
...
class SimulationDetails(ParametersBaseModel):
    """Provides comprehensive details about a Simulation's capabilities and identity.

    This model defines the core characteristics of a Simulation, including its
    ...
    simulation_id: ID = Field(
        ...,
        description="Identifier of the Simulation. Shall be unique within the scope of the CEM.",
    )
    name: Optional[str] = Field(None, description="Human readable name given by user.")
    roles: list[Role] = Field(..., description="Each Simulation provides one or more energy Roles.")
    ...
    available_control_types: list[ControlType] = Field(
        ..., description="The control types supported by this Simulation."
    )
    ...
    provides_forecast: bool = Field(
        ..., description="Indicates whether the Simulation is able to provide PowerForecasts."
    )
    provides_power_measurement_types: list[CommodityQuantity] = Field(
        ...,
        description="Array of all CommodityQuantities that this Simulation can provide measurements for.",
    )
...
class PEBCPowerEnvelope(ParametersBaseModel):
    ...
    id: ID = Field(
        ...,
        description=("Unique identifier of this PEBC.PowerEnvelope, scoped to the Simulation."),
    )
    commodity_quantity: CommodityQuantity = Field(
...
class PEBCInstruction(ParametersBaseModel):
    ...
    id: ID = Field(
        ..., description="Unique identifier of the instruction within the Simulation context."
    )
...
class PPBCPowerProfileDefinition(ParametersBaseModel):
    ...
    id: ID = Field(
        ...,
        description="Unique identifier of the PowerProfileDefinition within the Simulation session.",
    )
    start_time: DateTime = Field(
...
class OMBCOperationMode(ParametersBaseModel):
    ...
    id: ID = Field(..., description="Unique ID of the OperationMode within the Simulation session.")
    diagnostic_label: Optional[str] = Field(
...
class OMBCInstruction(ParametersBaseModel):
    ...
    id: ID = Field(..., description="Unique ID of the instruction within the Simulation session.")
    execution_time: DateTime = Field(
...
class FRBCActuatorDescription(ParametersBaseModel):
    ...
    id: ID = Field(..., description="Unique actuator ID within the Simulation session.")
    diagnostic_label: Optional[str] = Field(
...
class DDBCActuatorDescription(ParametersBaseModel):
    ...
    id: ID = Field(
        ...,
        description="ID of this actuator. Must be unique in the Simulation scope during the session.",
    )
    diagnostic_label: Optional[str] = Field(
        None, description="Human-readable name/description for diagnostics (not for HMI)."
    )
...
class DDBCInstruction(ParametersBaseModel):
    ...
    id: ID = Field(..., description="Unique identifier of the instruction in the Simulation scope.")
...
class SimulationBase(ConfigMixin, PredictionMixin, MeasurementMixin):
    """Base class for a simulation.

    Enables access to EOS configuration data (attribute `config`), EOS prediction data (attribute
    `prediction`) and EOS measurement data (attribute `measurement`).
    """

    def activate_control_type(self, control_type: ControlType) -> ReceptionStatus:
    ...
    def revoke_pebc_instruction(self, id: ID) -> ReceptionStatus:
        return ReceptionStatus(
            status=ReceptionStatusValues.REJECTED, diagnostic_label="abstract method"
        )

    def revoke_ppbc_instruction(self, id: ID) -> ReceptionStatus:
        return ReceptionStatus(
            status=ReceptionStatusValues.REJECTED, diagnostic_label="abstract method"
        )

    def revoke_ppbc_start_interrupt_instruction(self, id: ID) -> ReceptionStatus:
        return ReceptionStatus(
            status=ReceptionStatusValues.REJECTED, diagnostic_label="abstract method"
        )

    def revoke_ppbc_end_interrupt_instruction(self, id: ID) -> ReceptionStatus:
        return ReceptionStatus(
            status=ReceptionStatusValues.REJECTED, diagnostic_label="abstract method"
        )

    def revoke_ombc_instruction(self, id: ID) -> ReceptionStatus:
        return ReceptionStatus(
            status=ReceptionStatusValues.REJECTED, diagnostic_label="abstract method"
        )

    def revoke_frbc_instruction(self, id: ID) -> ReceptionStatus:
        return ReceptionStatus(
            status=ReceptionStatusValues.REJECTED, diagnostic_label="abstract method"
        )

    def revoke_ddbc_instruction(self, id: ID) -> ReceptionStatus:
        return ReceptionStatus(
            status=ReceptionStatusValues.REJECTED, diagnostic_label="abstract method"
        )
