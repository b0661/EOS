...
def PVForecast(predictions: pd.DataFrame, config: dict, date_time_tz: str) -> FT:
...
def ElectricityPriceForecast(predictions: pd.DataFrame, config: dict, date_time_tz: str) -> FT:
    source = ColumnDataSource(predictions)
...
def WeatherTempAirHumidityForecast(
    predictions: pd.DataFrame, config: dict, date_time_tz: str
) -> FT:
    source = ColumnDataSource(predictions)
...
def WeatherIrradianceForecast(predictions: pd.DataFrame, config: dict, date_time_tz: str) -> FT:
    source = ColumnDataSource(predictions)
    provider = config["weather"]["provider"]
...
def LoadForecast(predictions: pd.DataFrame, config: dict, date_time_tz: str) -> FT:
    source = ColumnDataSource(predictions)
...
def Prediction(eos_host: str, eos_port: Union[str, int]) -> str:
    ...
    return Grid(
        PVForecast(predictions, config, date_time_tz),
        ElectricityPriceForecast(predictions, config, date_time_tz),
        WeatherTempAirHumidityForecast(predictions, config, date_time_tz),
        WeatherIrradianceForecast(predictions, config, date_time_tz),
        LoadForecast(predictions, config, date_time_tz),
        cols_max=2,
    )
