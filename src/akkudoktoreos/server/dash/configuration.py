...
def Configuration(
    ...
    try:
        optimization_solution_keys = json.loads(config_details["optimization.keys"]["value"])
    except:
        optimization_solution_keys = []
    logger.debug(f"optimization_solution_keys: {optimization_solution_keys}")
    ...
    for config_key in sorted(config_details.keys()):
        ...
        if (
            ...
            config["name"].startswith("adapter.homeassistant.optimization_solution_entity_ids")
            and not config["deprecated"]
        ):
            ...
            rows.append(
                ConfigCard(
                    ...
                    update_value,
                    update_open,
                    make_config_update_map_form(optimization_solution_keys),
                )
            )
