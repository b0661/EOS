...
def JsonView(data: Any) -> Pre:
    ...
    return Pre(Code(code_str, cls="language-json"), cls="rounded-lg bg-muted overflow-auto")
    ...
    return Card(
        Details(
            ...
            Grid(
                ...
                # Provide minimal update form on error
                make_config_update_form()(config_name, update_value)
                if update_form_factory is not None
                else None,
            )
            if update_error
            else None,
