...
class HybridInverterDevice(EnergyDevice):
        ...
        if self.param.inverter_type in (InverterType.BATTERY, InverterType.HYBRID):
            ...
            if not (
                ...
                raise ValueError(
                    ...
                    f"[{self.param.battery_min_soc_factor}, {self.param.battery_max_soc_factor}]."
                )
    ...
    def create_batch_state(
        self, population_size: int, horizon: int
    ...
    def apply_genome_batch(
        ...
        for t in range(state.horizon):
            ...
            pv_dc_avail_w = (
                float(np.clip(self._pv_power_w[t], p.pv_min_power_w, p.pv_max_power_w))
    ...
    def apply_device_grant(
        self, state: HybridInverterBatchState, grant: DeviceGrant
    ) -> None:
        if self._step_interval_sec is None:
        ...
        * ``operation_mode_factor = pv_util_factor ∈ [0, 1]``
        ...
        for t, (bat_f, pv_f, dt) in enumerate(
            ...
            if (
                grid_wh is not None
                and battery_active
                and abs(grid_wh[t]) <= threshold_wh
            ):
    ...
    def _compute_ac_power(
        ...
        if charge_mask.any():
            ...
            if p.inverter_type in (InverterType.SOLAR, InverterType.HYBRID):
                ...
                ac_for_bat_w     = np.maximum(0.0, charge_bat_w - pv_for_bat) / p.ac_to_battery_efficiency
        ...
        if discharge_mask.any():
            ...
            dis_bat_ac_w = (-bf) * p.battery_max_discharge_rate * p.battery_capacity_wh * p.battery_to_ac_efficiency
    ...
    def _compute_min_energy(
        ...
        * For idle/PV-only steps: minimum is ``0`` (no battery constraint).
        ...
        min_dis_bat_ac_w = (
            p.battery_min_discharge_rate * p.battery_max_discharge_rate
            * p.battery_capacity_wh * p.battery_to_ac_efficiency
        )
