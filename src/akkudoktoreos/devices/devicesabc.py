...
@dataclass(frozen=True, slots=True)
class BatteryParam(DeviceParam):
    """Immutable battery parameters for simulation and GA caching.

    Attributes:
        device_id: Unique identifier for this battery.
        ports: Ports connecting this battery to energy buses.
        operation_modes: Supported operating modes for this battery.
        capacity_wh: Usable energy capacity [Wh].
        charging_efficiency: One-way charging efficiency [0..1].
        discharging_efficiency: One-way discharging efficiency [0..1].
        levelized_cost_of_storage_kwh: Amortised cost per kWh cycled [currency/kWh].
        max_charge_power_w: Maximum charge power [W].
        min_charge_power_w: Minimum charge power when active [W].
        charge_rates: Optional tuple of discrete charge rate fractions [0..1],
            or ``None`` if continuous control is used.
        min_soc_factor: Minimum allowed state of charge as a fraction of
            ``capacity_wh`` [0..1].
        max_soc_factor: Maximum allowed state of charge as a fraction of
            ``capacity_wh`` [0..1].
    """

    operation_modes: tuple[BatteryOperationMode, ...]
    capacity_wh: float
    ...
    charge_rates: tuple[float, ...] | None
    min_soc_factor: float
    max_soc_factor: float

    def __post_init__(self) -> None:
        if self.capacity_wh <= 0:
            raise ValueError("capacity_wh must be > 0")
        if self.max_charge_power_w <= 0:
            raise ValueError("max_charge_power_w must be > 0")
        if self.min_charge_power_w < 0:
            raise ValueError("min_charge_power_w must be >= 0")
        ...
        if not (0 < self.discharging_efficiency <= 1):
            raise ValueError("discharging_efficiency must be in (0, 1]")
        if not (0 <= self.min_soc_factor < self.max_soc_factor <= 1):
            raise ValueError("SoC factors must satisfy 0 <= min_soc_factor < max_soc_factor <= 1")
        if not self.operation_modes:
            raise ValueError("operation_modes must not be empty")


@dataclass(frozen=True, slots=True)
class InverterParam(DeviceParam):
    """Immutable inverter parameters.

    Attributes:
        device_id: Unique identifier for this inverter.
        ports: Ports connecting the inverter to DC and AC buses.
        max_power_w: Maximum AC output power [W].
        efficiency: DC-to-AC conversion efficiency [0..1].
    """
...
@dataclass(frozen=True, slots=True)
class PVParam(DeviceParam):
    """Immutable photovoltaic array parameters.

    Attributes:
        device_id: Unique identifier for this PV array.
        ports: Ports connecting the PV array to energy buses.
        peak_power_w: Peak power output [W] under standard test conditions.
        tilt_deg: Array tilt angle from horizontal [degrees].
        azimuth_deg: Array orientation [degrees], where 0° = North,
...
class HeatPumpParam(DeviceParam):
    ...
    operation_modes: tuple[ApplianceOperationMode, ...]
