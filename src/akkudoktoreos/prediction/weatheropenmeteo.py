...
class WeatherOpenMeteo(WeatherProvider):
    ...
    def _request_forecast(self) -> dict:
        ...
        days_diff = (end_dt - start_dt).days + 1  # +1 für inklusive

        # Open-Meteo hat ein Maximum von 16 Tagen
        forecast_days = min(days_diff, 16)

        # Entscheide ob wir forecast oder historical Daten brauchen
        now = to_datetime(in_timezone=self.config.general.timezone)

        if start_dt.date() >= now.date():
            # Zukünftige Daten - verwende forecast_days
            params["forecast_days"] = forecast_days
        else:
            # Historische Daten - verwende start_date und end_date
            params["start_date"] = start_dt.strftime("%Y-%m-%d")
            params["end_date"] = end_dt.strftime("%Y-%m-%d")
            # Für historische Daten müssen wir das forecast Modell spezifizieren
            params["models"] = "best_match"  # oder spezifisch z.B. "dwd", "icon", etc.
        ...
        # Wir arbeiten mit frischen Daten (kein Cache), Update-Zeit melden
    ...
    def _update_data(self, force_update: Optional[bool] = False) -> None:
        ...
        """
        # Open-Meteo Wetterdaten für die gegebenen Koordinaten abrufen
        openmeteo_data = self._request_forecast(force_update=force_update)  # type: ignore

        # Key-Mapping aus der Beschreibung erstellen
        ...
        # Zeitstempel und Werte aus Open-Meteo Antwort extrahieren
        ...
        for idx, timestamp in enumerate(timestamps):
            ...
            for openmeteo_key, item in openmeteo_key_mapping.items():
                ...
                # Wert aus hourly_data holen, falls vorhanden
        ...
        # Prüfen ob Strahlungswerte vorhanden sind (für Logging)
        ...
        if any(x is None or isinstance(x, float) and np.isnan(x) for x in temperature):
            # PWAT kann nicht berechnet werden
