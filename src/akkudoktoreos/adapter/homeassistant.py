...
SOLUTION_ENTITY_ID_PREFIX = "sensor.eos_"
...
class HomeAssistantAdapterCommonSettings(SettingsBaseModel):
    ...
    optimization_solution_entity_ids: HomeAssistantEntityIdMapping = Field(
        default_factory=HomeAssistantEntityIdMapping,
        json_schema_extra={
            "description": (
                "Mapping of EOS optimization solution to Home Assistant entity IDs.\n"
                f"The given IDs are prepended by '{SOLUTION_ENTITY_ID_PREFIX}' to build the entity_id.\n"
                f"E.g. 'my_op_mode' becomes the entity_id "
                f"'{SOLUTION_ENTITY_ID_PREFIX}_my_op_mode'."
            ),
            "examples": [
                {
                    "battery_operation_mode_id": "battery_operation_mode_id",
                    "battery_operation_mode_factor": "battery_operation_mode_factor",
                }
            ],
        },
    )
    ...
    def entity_ids(self) -> list[str]:
        ...
        try:
            ...
            result = adapter_eos.provider_by_id("HomeAssistant").get_entity_ids()
        except:
            return []
        return result
...
class HomeAssistantAdapter(AdapterProvider):
    ...
    def get_entity_ids(self) -> list[str]:
        ...
        if resp.ok:
            ...
            debug_msg = f"entity_ids: {entity_ids}"
    ...
    def _update_data(self) -> None:
        ...
        if stage == EnergyManagementStage.DATA_ACQUISITION:
            ...
            entity_ids = self.config.adapter.homeassistant.entity_id_battery_soc_factor
            if entity_ids:
                for index, entity_id in enumerate(entity_ids):
                    try:
                        ...
                        if state:
                            ...
                            resource_status = FRBCStorageStatus(
                                present_fill_level=float(soc_factor)
                            )
        ...
        if stage == EnergyManagementStage.CONTROL_DISPATCH:
            ...
            if optimization_solution:
                ...
                for (
                    solution_key,
                    entity_id,
                ) in self.config.adapter.homeassistant.optimization_solution_entity_ids.items():
                    try:
                        if entity_id:
                            entity_id = SOLUTION_ENTITY_ID_PREFIX + entity_id
