...
class AdapterProvider(
    SingletonMixin, ConfigMixin, MeasurementMixin, EnergyManagementSystemMixin, PydanticBaseModel
):
    ...
    def enabled(self) -> bool:
        ...
        if isinstance(self.config.adapter.provider, str):
            enabled = self.provider_id() == self.config.adapter.provider
        elif isinstance(self.config.adapter.provider, list):
            enabled = self.provider_id() in self.config.adapter.provider
        return enabled
...
class AdapterContainer(SingletonMixin, ConfigMixin, PydanticBaseModel):
    ...
    def provider_by_id(self, provider_id: str) -> AdapterProvider:
        """Retrieves a dmeasurement data provider by its unique identifier.
