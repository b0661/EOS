...
| provider | `EOS_ADAPTER__PROVIDER` | `Optional[list[Literal['HomeAssistant', 'NodeRED']]]` | `rw` | `None` | Adapter provider id(s) of provider(s) to be used [HomeAssistant, NodeRED, None]. |
:::
...
| entity_ids | `list[str]` | `ro` | `N/A` | Entity IDs available at Home Assistant. |
...
The given IDs are prepended by 'sensor.eos_' to build the entity_id.
E.g. 'my_op_mode' becomes the entity_id 'sensor.eos__my_op_mode'. |
:::
<!-- pyml enable line-length -->
